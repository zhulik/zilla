  ...
  class IoK8sApiCoreV1CSIVolumeSource
    ...
    def initialize(attributes = {})
      unless attributes.is_a?(Hash)
        raise ArgumentError,
              'The input argument (attributes) must be a hash in `Zilla::IoK8sApiCoreV1CSIVolumeSource` initialize method'
      end
      ...
      attributes = attributes.each_with_object({}) do |(k, v), h|
        unless self.class.attribute_map.key?(k.to_sym)
          raise ArgumentError,
                "`#{k}` is not a valid attribute in `Zilla::IoK8sApiCoreV1CSIVolumeSource`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
      ...
      return unless attributes.key?(:volume_attributes)
      return unless (value = attributes[:volume_attributes]).is_a?(Hash)
